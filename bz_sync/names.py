...
def resolve_bans(bans_by_oid: dict[int, dict[str, list[str]]], oid_to_name: dict[int, str],
    ...
    """oidStaff→근무표 이름 해석 + 시스템계정/제외 필터. off·half·reasons 보존, 동명이인 합집합."""
    ...
    for oid, rec in bans_by_oid.items():
        ...
        if mapped == "":
            continue
        slot = acc.setdefault(mapped, {"off": set(), "half": set(), "reasons": {}})
        slot["off"].update(rec.get("off", []))
        slot["half"].update(rec.get("half", []))
        slot["reasons"].update(rec.get("reasons", {}))
    return {name: {"off": sorted(v["off"]), "half": sorted(v["half"]), "reasons": v["reasons"]}
